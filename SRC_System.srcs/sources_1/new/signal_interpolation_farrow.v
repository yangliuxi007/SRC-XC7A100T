...
module signal_interpolation_farrow(
    ...
    wire [8:0]multiple_0;
    wire [8:0]multiple_1;
    wire signed [26:0]result_data[0:3]; //decimal places [14:0] 
    ...
    assign result_data[0] = (!sys_rstn) ? 27'd0 : ((data[0]*v3_3 + data[1]*v3_2 + data[2]*v3_1 + data[3]*v3_0)*uk*uk*uk);    
    assign result_data[1] = (!sys_rstn) ? 27'd0 : ((data[0]*v2_3 + data[1]*v2_2 + data[2]*v2_1 + data[3]*v2_0)*uk*uk); 
    assign result_data[2] = (!sys_rstn) ? 27'd0 : ((data[0]*v1_3 + data[1]*v1_2 + data[2]*v1_1 + data[3]*v1_0)*uk); 
    assign result_data[3] = (!sys_rstn) ? 27'd0 : (data[0]*v0_3 + data[1]*v0_2 + data[2]*v0_1 + data[3]*v0_0);       
    assign dout_inter_var = (!sys_rstn) ? 27'd0 : (flag_1 == 2'd1 ? (result_data[3] + result_data[2]/14'd10000 + result_data[1]/14'd10000/14'd10000 + result_data[0]/14'd10000/14'd10000/14'd10000) : 27'd0);     
    assign din_fifo = (!sys_rstn) ? 12'd0 : (flag_1 == 2'd1 ? dout_inter_var[26:15] : 12'd0);                                                                                                                                                                                                                                                                      
    ...
    always @(*) begin
        ...
        else if(wr_rst_busy_0 != 1'd1 && tab != 4'd0) begin
    ...
    always @(posedge module_clk or negedge sys_rstn) begin
        if(!sys_rstn)
            rd_en_0 <= 1'd0;
        else if(rd_rst_busy_0 != 1'd1) begin 
            if(cnt_0_0 < 2'd1 && !wr_en_0)
                rd_en_0 <= 1'd1;
            else
                rd_en_0 <= 1'd0;
        end
    ...
    always @(*) begin
        if(!sys_rstn)
            wr_en_1 <= 1'd0;
        else if(wr_rst_busy_1 != 1'd1 && tab != 4'd0) begin
    ...
    always @(posedge module_clk or negedge sys_rstn) begin
        if(!sys_rstn)
            rd_en_1 <= 1'd0;
        else if(rd_rst_busy_1 != 1'd1) begin 
            if(cnt_1_0 < 2'd1 && !wr_en_1)
                rd_en_1 <= 1'd1;
            else
                rd_en_1 <= 1'd0;
        end
    ...
    always @(posedge module_clk or negedge sys_rstn) begin
        if(!sys_rstn)
            rd_en_out_0 <= 1'd0;
        else if(rd_rst_busy_out_0 != 1'd1) begin 
            if(wr_data_count_out_0 == 10'd1022)
                rd_en_out_0 <= 1'd1;
            else if(wr_data_count_out_0 == 10'd1)
                rd_en_out_0 <= 1'd0;
            else
    ...
    always @(posedge module_clk or negedge sys_rstn) begin
        if(!sys_rstn)
            rd_en_out_1 <= 1'd0;
        else if(rd_rst_busy_out_1 != 1'd1) begin 
            if(wr_data_count_out_1 == 10'd1022)
                rd_en_out_1 <= 1'd1;
            else if(wr_data_count_out_1 == 10'd1)
                rd_en_out_1 <= 1'd0;
            else
    ...
    ////////// ip core part
    fifo_farrow_0 fifo_farrow_0_inst (
    ...
    fifo_farrow_1 fifo_farrow_1_inst (
        .rst(!sys_rstn),                  // input wire rst
        .wr_clk(sample_clk),              // input wire wr_clk
        .rd_clk(module_clk),              // input wire rd_clk
        .din(din),                        // input wire [11 : 0] din
    ...
    fifo_farrow_out fifo_farrow_out0 (
        .rst(!sys_rstn),                      // input wire rst
        ...
        .wr_rst_busy(wr_rst_busy_out_0),      // output wire wr_rst_busy
        .rd_rst_busy(rd_rst_busy_out_0)      // output wire rd_rst_busy
    );  

    fifo_farrow_out fifo_farrow_out1 (
        .rst(!sys_rstn),                      // input wire rst
        .wr_clk(module_clk),                // input wire wr_clk
        .rd_clk(conver_clk),                // input wire rd_clk
        .din(din_fifo),                      // input wire [11 : 0] din
        ...
        .wr_rst_busy(wr_rst_busy_out_1),      // output wire wr_rst_busy
        .rd_rst_busy(rd_rst_busy_out_1)      // output wire rd_rst_busy
    ); 
